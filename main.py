...
for hyperparameter in tqdm.tqdm(hyp_comb,colour="yellow", desc="Tried combinations"):
    ...
    trainer = pl.Trainer(log_every_n_steps=20,max_epochs = num_epochs,callbacks=[EarlyStopping(monitor="val_loss", patience=3,mode='min'), ModelCheckpoint(filename= filename,monitor='val_loss',save_top_k=1,every_n_epochs=1,mode='min',save_weights_only=True,verbose=True,dirpath=utils.CKPT_SAVE_DIR_NAME)],logger=logger,accelerator='gpu')
